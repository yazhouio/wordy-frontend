...
const ActionButton = (props: ActionButtonProps) => {
        ...
        switchMap((loading) => {
          ...
          return message$.pipe(
            filter((message: WsRequest) => {
              return (
                message.eventType === EventType.Speech &&
  ...
  return (
    <Button
      onClick={handleClick}
      isLoading={isLoading}
      startContent={<Cmp className="flex-none" size={20} />}
    >
...
const ChatItem = (props: ChatItemProps) => {
  ...
  const lines = message.split("\n").map((line, index) => {
    return (
      <div className="indent-8 text-medium" key={index}>
        {line}
      </div>
    );
  });
  const renderMore = () => {
    const list = message.match(/`\w.*?`/g)?.map((text, index) => {
      return (
        <ActionButton
    ...
    return (
      <div className={"grid gap-3 chat-play-button"}>
  ...
  return (
    <div
      className={clsx("flex gap-1 mb-8", {
    ...
    >
      ...
      {!isMe ? (
        <Accordion className="w-4/5" variant="splitted">
